...
(function() {
  ...
  _.each = function(collection, iterator) {
    // Handle arrays differently then objects
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(value, key, collection){
      var invokeFunc;
      if(typeof(functionOrKey) === "function") { //function passed is a function, use it.
        invokeFunc = functionOrKey;
  ...
  _.some = function(collection, iterator) {
    ...
    return !_.every(collection, function(item){ //returns true if not every items is false
  ...
  _.extend = function(obj) {
    _.each(arguments, function(objFrom){
      if(!(objFrom == obj)) {
        _.each(objFrom, function(val, key){
          obj[key] = objFrom[key];
        }); 
      }
    });
  ...
  _.memoize = function(func) {
    ...
    return function() {
      var argKey = arguments[0].toString();
      if (!(argKey in resultCache)) { //We've not seen this argument before
        resultCache[argKey] = func.apply(this, arguments);
      }
      return resultCache[argKey];
    };
  };
  ...
  _.delay = function(func, wait) {
    var funcArgs = Array.prototype.slice.call(arguments).slice(2);
  ...
  _.shuffle = function(array) {
    ...
    for(var i = shuffledArr.length - 1; i >= 1; i--) {
      var j = Math.floor(Math.random() * i);
        swapTemp = shuffledArr[j];
        shuffledArr[j] = shuffledArr[i];
        shuffledArr[i] = swapTemp;
    }
    return shuffledArr;
  };
